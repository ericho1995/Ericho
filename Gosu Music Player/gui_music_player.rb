...
class MusicPlayerMain < Gosu::Window
  ...
  # Draws the artwork on the screen for all the albums
	...
	def draw_albums(albums)
	    ...
	    @art.draw(50, 50 , z = ZOrder::PLAYER)
		
	    @art = Gosu::Image.new(albums[1].icon)
		@art.draw(50, 340, z = ZOrder::PLAYER)#nectar

	    @art= Gosu::Image.new(albums[2].icon)#slow dancing
	    @art.draw(310, 50 , z = ZOrder::PLAYER)
   
	    @art = Gosu::Image.new(albums[3].icon)
		@art.draw(310, 340, z = ZOrder::PLAYER)#marvins room
		
		@art = Gosu::Image.new(albums[4].icon)
		@art.draw(570, 50, z = ZOrder::PLAYER)#league of legends
...
# Not used? Everything depends on mouse actions.
 ...
 # Draws the album images and the track list for the selected album
	...
	def print_genre(albums)
		...
		begin
		x = 1
		while (x < Genre_names.length)
		  puts (x.to_s + " - " + Genre_names[x])
		  x += 1
		end
		puts "5 - Return"
	 ...
	 def print_rnb(albums)
		x = 0
		while (x < albums.length) 
		  if (albums[x].genre == Genre_names[4])
	 ...
	 def print_soul(albums)
		x = 0
		while (x < albums.length) 
		  if (albums[x].genre == Genre_names[3])
			  print_album(albums[x])
		  end
		  x += 1
		end
	 ...
	 def print_hiphop(albums)
		x = 0
		while (x < albums.length) 
		  if (albums[x].genre == Genre_names[2])
			  print_album(albums[x])
		  end
		  x += 1
		end
	 ...
	 def print_pop(albums)
		x = 0
		while (x < albums.length) 
		  if (albums[x].genre == Genre_names[1])
		  print_album(albums[x])
		  end
		  x += 1
		end
	 ...
	 def print_album(albums)
		puts "Album ID: " + (albums.id)
		puts "Genre: " + (albums.genre)
		puts "Title: " + (albums.title)
		puts "Artist: " + (albums.artist)
	 end
